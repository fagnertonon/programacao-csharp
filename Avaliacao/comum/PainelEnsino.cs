...
namespace Conecta
{
    ...
    public static class UI
    {
        ...
        // Poe o conteudo no meio do painel. Em tela cheia sobra largura dos dois
        // lados; sem isto o texto fica grudado na esquerda e a metade direita
        // parece um erro de montagem.
        public static void Centralizar(Panel destino)
        {
            foreach (Control c in destino.Controls)
            {
                FlowLayoutPanel f = c as FlowLayoutPanel;
                if (f == null) { continue; }

                int sobra = destino.ClientSize.Width - f.Width;
                f.Left = sobra > 0 ? sobra / 2 : 0;
            }
        }
    ...
    public static class PainelEnsino
    {
        ...
        // O texto da faixa ambar vem do JSON: "os outros sete sao revisao" so
        // valia para a aula em que havia um assunto novo entre oito abas.
        public static void Montar(Panel destino, Topico t, int largura, string avisoNovo)
        {
            ...
            if (t.Novo)
            {
                fluxo.Controls.Add(UI.Aviso(
                    ...
                    string.IsNullOrEmpty(avisoNovo)
                        ? "Este assunto e novo - aprenda com calma, sem correr."
                        : avisoNovo,
                    Paleta.Amber, Paleta.AmberBg, largura));
            }

            // Os tres blocos abaixo sao opcionais no JSON. Sem estas guardas,
            // um topico incompleto derrubava o aplicativo na abertura.
            if (t.Explicacao != null)
            {
            ...
            if (!string.IsNullOrEmpty(t.Diagrama))
            {
            ...
            if (!string.IsNullOrEmpty(t.NoSeuProjeto))
            {
                fluxo.Controls.Add(UI.Aviso("NO SEU PROJETO", t.NoSeuProjeto,
                                            Paleta.Purple, Paleta.Tint, largura));
            }
            ...
            if (!string.IsNullOrEmpty(t.FolhaRevisao))
            {
