    ...
    public class Diagrama : Panel
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            switch (tipo)
            {
                ...
                // Nome errado no JSON desenhava um painel branco, em silencio,
                // e quem escreveu o conteudo achava que o diagrama tinha sumido.
                default: Faltando(g); break;
            }
        }
        ...
        // So aparece quando o nome do diagrama no JSON nao existe aqui.
        private void Faltando(Graphics g)
        {
            if (string.IsNullOrEmpty(tipo)) { return; }

            using (SolidBrush t = new SolidBrush(Paleta.Red))
            {
                g.DrawString("diagrama nao encontrado: \"" + tipo + "\"",
                             FonteMonoB, t, 16, 16);
            }
        }
        ...
        {
            Caixa(g, 12, 20, 200, 110, "class Usuario  (o molde)",
                  new string[] { "int Id", "string Nome", "string Login", "string Senha" },
            ...
            Nota(g, 470, 55, "Um molde, duas contas.", Paleta.Body);
            Nota(g, 470, 75, "Cada uma com o seu Login.", Paleta.Body);
        }
        ...
        private void AssinaturaMetodo(Graphics g)
        {
            using (SolidBrush t = new SolidBrush(Paleta.Ink))
            {
                ...
                g.DrawString("Usuario", FonteMonoB, t, 80, 60);
                g.DrawString("BuscarPorId", FonteMonoB, t, 148, 60);
                g.DrawString("(", FonteMono, t, 245, 60);
                g.DrawString("int id", FonteMonoB, t, 255, 60);
                g.DrawString(")", FonteMono, t, 310, 60);
            }

            Seta(g, 110, 100, 110, 78, "");
            Nota(g, 52, 102, "o que DEVOLVE", Paleta.Purple);

            Seta(g, 196, 40, 196, 58, "");
            Nota(g, 170, 20, "nome", Paleta.Purple);

            Seta(g, 278, 100, 278, 78, "");
            Nota(g, 238, 102, "PARAMETRO", Paleta.Purple);

            Nota(g, 400, 40, "Na chamada:", Paleta.Body);
            using (SolidBrush t = new SolidBrush(Paleta.Ink))
            {
                g.DrawString("dao.BuscarPorId(1);", FonteMonoB, t, 400, 60);
            }
            Seta(g, 543, 100, 543, 80, "");
            Nato(g, 505, 102, "ARGUMENTO");

            Nota(g, 400, 128, "Devolve a conta, ou null se nao existir.", Paleta.Body);
        }
        ...
        private void TabelaVerdade(Graphics g)
        {
            ...
            Nota(g, 320, 96, "O login usa os DOIS ao mesmo tempo:", Paleta.Body);
            using (SolidBrush t = new SolidBrush(Paleta.Ink))
            {
                g.DrawString("if (u.Login == login &&", FonteMonoB, t, 320, 116);
                g.DrawString("    u.Senha == senha)", FonteMonoB, t, 320, 136);
            }
        }
        ...
        private void ForTresPartes(Graphics g)
        {
            using (SolidBrush t = new SolidBrush(Paleta.Ink))
            {
                ...
                g.DrawString("i < Count", FonteMonoB, t, 160, 70);
            ...
            Nota(g, 400, 44, "i vale 0, 1, 2 ...", Paleta.Mint);
            Nota(g, 400, 70, "e a conta numero i + 1.", Paleta.Amber);
        ...
        {
            Nota(g, 20, 14, "List<Usuario> contas", Paleta.Purple);
            ...
            Nota(g, 400, 118, "contas[0] e a primeira.", Paleta.Red);
        }
    }
}
