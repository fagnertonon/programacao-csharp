    ...
    public static class Corretor
    {
        ...
        private static string Executar(string idTopico, string entrada)
        {
            ...
            if (idTopico == "contar")
            {
                ...
                int minimo = 0;
                int.TryParse(Arg(p, 1), out minimo);
                return Desafios.ContarSenhasFracas(contas, minimo).ToString();
            }

            if (idTopico == "atalho")
            {
                return Desafios.TelaDoAtalho(Arg(p, 0));
            }

            if (idTopico == "media")
            {
                return Desafios.MediaTamanhoSenha(LerContas(Arg(p, 0))).ToString();
            }

            if (idTopico == "linha")
            {
                return Desafios.ListarLogins(LerContas(Arg(p, 0)));
            }

            if (idTopico == "campeao")
            {
                // O separador aqui e " / " e nao "|": a barra vertical ja e o
                // separador de argumentos da entrada, e reusar confunde a leitura.
                Usuario c = Desafios.NomeMaisLongo(LerContas(Arg(p, 0)));
                if (c == null) { return "(null)"; }
                return c.Id + " / " + c.Nome + " / " + c.Login;
            }

            if (idTopico == "resumo")
            {
                return Desafios.ResumoDaTurma(LerContas(Arg(p, 0)));
            }

            if (idTopico == "inverso")
            {
                return Desafios.Inverter(LerContas(Arg(p, 0)));
            }

            if (idTopico == "vizinho")
            {
                return Desafios.LoginIgualAoAnterior(LerContas(Arg(p, 0)));
            }

            if (idTopico == "posicao")
            {
                return Desafios.PosicaoDoLogin(LerContas(Arg(p, 0)), Arg(p, 1)).ToString();
            }

            if (idTopico == "importar")
            {
                List<Usuario> contas = LerContas(Arg(p, 0));
                List<Usuario> chegando = LerContas(Arg(p, 1));

                int n = Desafios.Importar(contas, chegando);

                // O estado da lista DEPOIS da chamada e o que pega quem devolve
                // o numero certo sem acrescentar ninguem.
                string quem = "";
